...
- [x] Add deps: `drizzle-orm`, `postgres`; dev `drizzle-kit`, `vitest`, Node types. (`packages/db/package.json`)
- [x] **Write failing test** `rls.test.ts`: connect as app role, set org A, insert repo(orgA) + repo(orgB) via superuser seed, assert org-A session sees only its row.
- [x] Run: `pnpm --filter @agent-git/db test` → FAIL (schema/client missing).
- [x] Implement `id.ts` (UUIDv7 helper), `schema/*` (Drizzle tables, `org_id` where tenant-scoped), `client.ts` (drizzle + per-tx `set_config`).
- [x] Create migration SQL: `0000_init.sql` for tables + `0001_rls.sql` for `agentgit_app`, RLS policies, grants, plus Drizzle journal metadata.
- [x] Apply: `pnpm --filter @agent-git/db db:migrate` (against `pnpm dev:infra`).
- [x] Run test → PASS.
- [x] Update `packages/db/AGENTS.md` structure; worklog.
